...
export default function Header() {
    ...
    fetch(`${apiUrl}/api/categories`, { signal: ctrl.signal })
      ...
      .then((json) => {
        if (json?.success && Array.isArray(json.data)) {
          ...
          const ordered = CAT_ORDER.map((slug) =>
            top.find((c: CategoryItem) => c.slug === slug),
          ).filter(Boolean) as CategoryItem[];
          const others = top.filter(
            (c: CategoryItem) => !CAT_ORDER.includes(c.slug),
          );
      ...
      try {
        ...
        if (res.ok) {
          ...
          const total = items.reduce(
            (s: number, i: { soLuong: number }) => s + i.soLuong,
            0,
          );
          setCartCount(total);
        }
  ...
  const showDropdown =
    showTrending || showResults || (suggestLoading && !!searchInput.trim());
        ...
        >
          <div className="max-w-screen-xl mx-auto px-6 flex items-center justify-between h-8">
            <p className="text-[11.5px] text-red-100 font-medium tracking-wide">
              <span className="text-yellow-300 font-semibold">SmartHub</span> —
              Công nghệ đỉnh cao, giá cả hợp lý, giao hàng tận nơi!
            </p>
            <div className="flex items-center gap-4">
              ...
              <a
                href="tel:18009999"
                className="flex items-center gap-1 text-[11px] text-yellow-300 font-semibold hover:text-yellow-200 transition-colors"
              >
                    ...
                    suggestions.length === 0 && (
                      <div className="px-4 py-6 text-center">
                        <p className="text-[13px] text-gray-400">
                          Không tìm thấy sản phẩm cho &ldquo;{searchInput}
                          &rdquo;
                        </p>
                      </div>
                    )}
        ...
        {mobileOpen && (
          <div className="md:hidden bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-slate-700 px-6 py-5 space-y-3 shadow-lg">
            ...
            <form
              ...
              className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 focus-within:border-red-400 focus-within:bg-white transition-colors"
            >
            ...
            <div className="space-y-1">
              {mobileCategories.length === 0
                ? Array.from({ length: 5 }).map((_, i) => (
                    <div
                      key={i}
                      className="h-10 rounded-sm bg-gray-100 animate-pulse"
                    />
                  ))
